...
//#endregion
//#region node_modules/ml-matrix/matrix.mjs
var import_matrix = /* @__PURE__ */ __toESM((/* @__PURE__ */ __commonJSMin(((exports) => {
	...
	*/
	function max(input, options = {}) {
		if (!isAnyArray(input)) throw new TypeError("input must be an array");
		...
		for (let i = fromIndex + 1; i < toIndex; i++) if (input[i] > maxValue) maxValue = input[i];
		return maxValue;
	}
	/**
	...
	*/
	function min(input, options = {}) {
		...
		for (let i = fromIndex + 1; i < toIndex; i++) if (input[i] < minValue) minValue = input[i];
		return minValue;
	}
	/**
	...
	*/
	function rescale(input, options = {}) {
		...
		if (options.output !== void 0) {
			if (!isAnyArray(options.output)) throw new TypeError("output option must be an array if specified");
		...
		for (let i = 0; i < input.length; i++) output[i] = (input[i] - currentMin) * factor + minValue;
		return output;
	}
	var indent = " ".repeat(2);
...
${indent}columns: ${matrix.columns}
	...
	function inspectData(matrix, maxRows, maxColumns, maxNumSize, padMinus) {
		...
		if (padMinus === "auto") {
			padMinus = false;
			loop: for (let i = 0; i < maxI; i++) for (let j = 0; j < maxJ; j++) if (matrix.get(i, j) < 0) {
				padMinus = true;
				break loop;
			}
		}
		for (let i = 0; i < maxI; i++) {
			let line = [];
			for (let j = 0; j < maxJ; j++) line.push(formatNumber(matrix.get(i, j), maxNumSize, padMinus));
	...
	function installMathOperations(AbstractMatrix, Matrix) {
		...
		AbstractMatrix.add = function add(matrix, value) {
			return new Matrix(matrix).add(value);
		};
		...
		AbstractMatrix.sub = function sub(matrix, value) {
			return new Matrix(matrix).sub(value);
		};
		...
		AbstractMatrix.mul = function mul(matrix, value) {
			return new Matrix(matrix).mul(value);
		};
		...
		AbstractMatrix.mod = function mod(matrix, value) {
			return new Matrix(matrix).mod(value);
		};
		...
		AbstractMatrix.and = function and(matrix, value) {
			return new Matrix(matrix).and(value);
		};
		...
		AbstractMatrix.or = function or(matrix, value) {
			return new Matrix(matrix).or(value);
		};
		...
		AbstractMatrix.xor = function xor(matrix, value) {
			return new Matrix(matrix).xor(value);
		};
		...
		AbstractMatrix.leftShift = function leftShift(matrix, value) {
			return new Matrix(matrix).leftShift(value);
		};
		...
		AbstractMatrix.signPropagatingRightShift = function signPropagatingRightShift(matrix, value) {
			return new Matrix(matrix).signPropagatingRightShift(value);
		};
		...
		AbstractMatrix.rightShift = function rightShift(matrix, value) {
			return new Matrix(matrix).rightShift(value);
		};
		...
		AbstractMatrix.acosh = function acosh(matrix) {
			return new Matrix(matrix).acosh();
		};
		...
		AbstractMatrix.asinh = function asinh(matrix) {
			return new Matrix(matrix).asinh();
		};
		...
		AbstractMatrix.atanh = function atanh(matrix) {
			return new Matrix(matrix).atanh();
		};
		...
		AbstractMatrix.clz32 = function clz32(matrix) {
			return new Matrix(matrix).clz32();
		};
		...
		AbstractMatrix.expm1 = function expm1(matrix) {
			return new Matrix(matrix).expm1();
		};
		...
		AbstractMatrix.floor = function floor(matrix) {
			return new Matrix(matrix).floor();
		};
		...
		AbstractMatrix.fround = function fround(matrix) {
			return new Matrix(matrix).fround();
		};
		...
		AbstractMatrix.log1p = function log1p(matrix) {
			return new Matrix(matrix).log1p();
		};
		...
		AbstractMatrix.log10 = function log10(matrix) {
			return new Matrix(matrix).log10();
		};
		...
		AbstractMatrix.round = function round(matrix) {
			return new Matrix(matrix).round();
		};
		...
		AbstractMatrix.trunc = function trunc(matrix) {
			return new Matrix(matrix).trunc();
		};
		AbstractMatrix.pow = function pow(matrix, arg0) {
			return new Matrix(matrix).pow(arg0);
		};
	...
	*/
	...
	function checkRowIndices(matrix, rowIndices) {
		if (!isAnyArray(rowIndices)) throw new TypeError("row indices must be an array");
		for (let i = 0; i < rowIndices.length; i++) if (rowIndices[i] < 0 || rowIndices[i] >= matrix.rows) throw new RangeError("row indices are out of range");
	}
	function checkColumnIndices(matrix, columnIndices) {
		if (!isAnyArray(columnIndices)) throw new TypeError("column indices must be an array");
		for (let i = 0; i < columnIndices.length; i++) if (columnIndices[i] < 0 || columnIndices[i] >= matrix.columns) throw new RangeError("column indices are out of range");
	}
	...
	var AbstractMatrix = class AbstractMatrix {
		static from1DArray(newRows, newColumns, newData) {
			if (newRows * newColumns !== newData.length) throw new RangeError("data length does not match given dimensions");
		...
		isSymmetric() {
			if (this.isSquare()) {
				for (let i = 0; i < this.rows; i++) for (let j = 0; j <= i; j++) if (this.get(i, j) !== this.get(j, i)) return false;
				return true;
			}
			return false;
		}
		...
		isEchelonForm() {
			...
			while (i < this.rows && isEchelonForm) {
				j = 0;
				checked = false;
				while (j < this.columns && checked === false) if (this.get(i, j) === 0) j++;
		...
		max(by) {
			...
			switch (by) {
				case "row": {
					...
					for (let row = 0; row < this.rows; row++) for (let column = 0; column < this.columns; column++) if (this.get(row, column) > max[row]) max[row] = this.get(row, column);
					return max;
				}
				case "column": {
					const max = new Array(this.columns).fill(Number.NEGATIVE_INFINITY);
					for (let row = 0; row < this.rows; row++) for (let column = 0; column < this.columns; column++) if (this.get(row, column) > max[column]) max[column] = this.get(row, column);
					return max;
				}
				case void 0: {
					let max = this.get(0, 0);
					for (let row = 0; row < this.rows; row++) for (let column = 0; column < this.columns; column++) if (this.get(row, column) > max) max = this.get(row, column);
					return max;
				}
				default: throw new Error(`invalid option: ${by}`);
			}
		}
		maxIndex() {
			checkNonEmpty(this);
			let v = this.get(0, 0);
			let idx = [0, 0];
			for (let i = 0; i < this.rows; i++) for (let j = 0; j < this.columns; j++) if (this.get(i, j) > v) {
				v = this.get(i, j);
				idx[0] = i;
				idx[1] = j;
			}
		...
		min(by) {
			...
			switch (by) {
				case "row": {
					...
					for (let row = 0; row < this.rows; row++) for (let column = 0; column < this.columns; column++) if (this.get(row, column) < min[row]) min[row] = this.get(row, column);
					return min;
				}
				case "column": {
					const min = new Array(this.columns).fill(Number.POSITIVE_INFINITY);
					for (let row = 0; row < this.rows; row++) for (let column = 0; column < this.columns; column++) if (this.get(row, column) < min[column]) min[column] = this.get(row, column);
					return min;
				}
				case void 0: {
					let min = this.get(0, 0);
					for (let row = 0; row < this.rows; row++) for (let column = 0; column < this.columns; column++) if (this.get(row, column) < min) min = this.get(row, column);
					return min;
				}
				default: throw new Error(`invalid option: ${by}`);
			}
		...
		norm(type = "frobenius") {
			switch (type) {
				...
				default: throw new RangeError(`unknown norm type: ${type}`);
			}
		}
		cumulativeSum() {
			let sum = 0;
		...
		mmulStrassen(y) {
			...
			let r2 = y.rows;
			let c2 = y.columns;
			if (c1 !== r2) console.warn(`Multiplying ${r1} x ${c1} and ${r2} x ${c2} matrix: dimensions do not match.`);
			...
			function blockMult(a, b, rows, cols) {
				if (rows <= 512 || cols <= 512) return a.mmul(b);
		...
		scaleColumns(options = {}) {
			...
			for (let i = 0; i < this.columns; i++) {
				...
				if (column.length) rescale(column, {
					min,
					max,
					output: column
				});
		...
		kroneckerProduct(other) {
			...
			for (let i = 0; i < m; i++) for (let j = 0; j < n; j++) for (let k = 0; k < p; k++) for (let l = 0; l < q; l++) result.set(p * i + k, q * j + l, this.get(i, j) * other.get(k, l));
			return result;
		}
		kroneckerSum(other) {
			other = Matrix.checkMatrix(other);
		...
		*/
		...
		sum(by) {
			switch (by) {
				case "row": return sumByRow(this);
				case "column": return sumByColumn(this);
				case void 0: return sumAll(this);
				default: throw new Error(`invalid option: ${by}`);
			}
		}
		product(by) {
			switch (by) {
				case "row": return productByRow(this);
				case "column": return productByColumn(this);
				case void 0: return productAll(this);
				default: throw new Error(`invalid option: ${by}`);
			}
		}
		mean(by) {
			...
			switch (by) {
				...
				case "column":
					...
					return sum;
				case void 0: return sum / this.size;
				default: throw new Error(`invalid option: ${by}`);
			}
		}
		variance(by, options = {}) {
			...
			switch (by) {
				case "row":
					if (!isAnyArray(mean)) throw new TypeError("mean must be an array");
		...
		standardDeviation(by, options) {
			...
			if (by === void 0) return Math.sqrt(variance);
			else {
		...
		center(by, options = {}) {
			...
			switch (by) {
				case "row":
					if (!isAnyArray(center)) throw new TypeError("center must be an array");
		...
		scale(by, options = {}) {
			...
			switch (by) {
				case "row":
					if (scale === void 0) scale = getScaleByRow(this);
					else if (!isAnyArray(scale)) throw new TypeError("scale must be an array");
					scaleByRow(this, scale);
					return this;
				case "column":
					if (scale === void 0) scale = getScaleByColumn(this);
					else if (!isAnyArray(scale)) throw new TypeError("scale must be an array");
					scaleByColumn(this, scale);
					return this;
				case void 0:
					if (scale === void 0) scale = getScaleAll(this);
	...
	if (typeof Symbol !== "undefined") AbstractMatrix.prototype[Symbol.for("nodejs.util.inspect.custom")] = inspectMatrix;
	...
	var Matrix = class Matrix extends AbstractMatrix {
		/**
		* @type {Float64Array[]}
		*/
		data;
		...
		constructor(nRows, nColumns) {
			...
			if (Matrix.isMatrix(nRows)) {
				...
				for (let i = 0; i < nRows; i++) {
					...
					if (!isArrayOfNumbers(arrayData[i])) throw new TypeError("Input data contains non-numeric values");
	...
	*/
	var SymmetricMatrix = class SymmetricMatrix extends AbstractMatrix {
		/** @type {Matrix} */
		#matrix;
		get size() {
		...
		*/
		constructor(diagonalSize) {
			...
			if (Matrix.isMatrix(diagonalSize)) {
				if (!diagonalSize.isSymmetric()) throw new TypeError("not symmetric data");
				this.#matrix = Matrix.copy(diagonalSize, new Matrix(diagonalSize.rows, diagonalSize.rows));
			} else if (Number.isInteger(diagonalSize) && diagonalSize >= 0) this.#matrix = new Matrix(diagonalSize, diagonalSize);
			else {
				this.#matrix = new Matrix(diagonalSize);
				if (!this.isSymmetric()) throw new TypeError("not symmetric data");
			}
		}
		clone() {
			const matrix = new SymmetricMatrix(this.diagonalSize);
			for (const [row, col, value] of this.upperRightEntries()) matrix.set(row, col, value);
			return matrix;
		}
		toMatrix() {
		...
		*/
		applyMask(mask) {
			if (mask.length !== this.diagonalSize) throw new RangeError("Mask size do not match with matrix size");
			/** @type {number[]} */
			const sidesToRemove = [];
			...
			for (const sideIndex of sidesToRemove) this.removeCross(sideIndex);
			return this;
		}
		/**
		...
		*/
		static fromCompact(compact) {
			...
			if (!Number.isInteger(diagonalSize)) throw new TypeError(`This array is not a compact representation of a Symmetric Matrix, ${JSON.stringify(compact)}`);
		...
		*/
		*upperRightEntries() {
			for (let row = 0, col = 0; row < this.diagonalSize;) {
	...
	var DistanceMatrix = class DistanceMatrix extends SymmetricMatrix {
		/**
		* not the same as matrix.isSymmetric()
		* Here is to check if it's instanceof SymmetricMatrix without bundling issues
		*
		...
		*/
		...
		constructor(sideSize) {
			super(sideSize);
			if (!this.isDistance()) throw new TypeError("Provided arguments do no produce a distance matrix");
		}
		set(rowIndex, columnIndex, value) {
		...
		*/
		static fromCompact(compact) {
			const compactSize = compact.length;
			if (compactSize === 0) return new this(0);
			const diagonalSize = (Math.sqrt(8 * compactSize + 1) + 1) / 2;
			if (!Number.isInteger(diagonalSize)) throw new TypeError(`This array is not a compact representation of a DistanceMatrix, ${JSON.stringify(compact)}`);
	...
	var WrapperMatrix1D = class extends AbstractMatrix {
		constructor(data, options = {}) {
			...
			if (data.length % rows !== 0) throw new Error("the data length is not divisible by the number of rows");
			super();
			this.rows = rows;
	...
	function wrap(array, options) {
		if (isAnyArray(array)) if (array[0] && isAnyArray(array[0])) return new WrapperMatrix2D(array);
		else return new WrapperMatrix1D(array, options);
		else throw new Error("the argument is not an array");
	}
	var LuDecomposition = class {
		constructor(matrix) {
			...
			let i, j, k, p, s, t, v;
			let LUcolj, kmax;
			for (i = 0; i < rows; i++) pivotVector[i] = i;
			LUcolj = new Float64Array(rows);
			for (j = 0; j < columns; j++) {
				for (i = 0; i < rows; i++) LUcolj[i] = lu.get(i, j);
				for (i = 0; i < rows; i++) {
					kmax = Math.min(i, j);
					s = 0;
					for (k = 0; k < kmax; k++) s += lu.get(i, k) * LUcolj[k];
					LUcolj[i] -= s;
					lu.set(i, j, LUcolj[i]);
				}
				p = j;
				for (i = j + 1; i < rows; i++) if (Math.abs(LUcolj[i]) > Math.abs(LUcolj[p])) p = i;
				if (p !== j) {
					for (k = 0; k < columns; k++) {
						t = lu.get(p, k);
				...
				if (j < rows && lu.get(j, j) !== 0) for (i = j + 1; i < rows; i++) lu.set(i, j, lu.get(i, j) / lu.get(j, j));
			}
			this.LU = lu;
		...
		isSingular() {
			...
			for (let j = 0; j < col; j++) if (data.get(j, j) === 0) return true;
			return false;
		}
		solve(value) {
			value = Matrix.checkMatrix(value);
			let lu = this.LU;
			if (lu.rows !== value.rows) throw new Error("Invalid matrix dimensions");
			if (this.isSingular()) throw new Error("LU matrix is singular");
			let count = value.columns;
			let X = value.subMatrixRow(this.pivotVector, 0, count - 1);
			let columns = lu.columns;
			let i, j, k;
			for (k = 0; k < columns; k++) for (i = k + 1; i < columns; i++) for (j = 0; j < count; j++) X.set(i, j, X.get(i, j) - X.get(k, j) * lu.get(i, k));
			for (k = columns - 1; k >= 0; k--) {
				for (j = 0; j < count; j++) X.set(k, j, X.get(k, j) / lu.get(k, k));
				for (i = 0; i < k; i++) for (j = 0; j < count; j++) X.set(i, j, X.get(i, j) - X.get(k, j) * lu.get(i, k));
			}
			return X;
		}
		get determinant() {
			...
			let col = data.columns;
			for (let j = 0; j < col; j++) determinant *= data.get(j, j);
			return determinant;
		}
		get lowerTriangularMatrix() {
			...
			for (let i = 0; i < rows; i++) for (let j = 0; j < columns; j++) if (i > j) X.set(i, j, data.get(i, j));
			else if (i === j) X.set(i, j, 1);
			else X.set(i, j, 0);
			return X;
		}
	...
	var QrDecomposition = class {
		constructor(value) {
			...
			for (k = 0; k < n; k++) {
				let nrm = 0;
				for (i = k; i < m; i++) nrm = hypotenuse(nrm, qr.get(i, k));
				if (nrm !== 0) {
					if (qr.get(k, k) < 0) nrm = -nrm;
					for (i = k; i < m; i++) qr.set(i, k, qr.get(i, k) / nrm);
					qr.set(k, k, qr.get(k, k) + 1);
					for (j = k + 1; j < n; j++) {
						s = 0;
						for (i = k; i < m; i++) s += qr.get(i, k) * qr.get(i, j);
						s = -s / qr.get(k, k);
						for (i = k; i < m; i++) qr.set(i, j, qr.get(i, j) + s * qr.get(i, k));
					}
				}
				rdiag[k] = -nrm;
			}
		...
		solve(value) {
			...
			for (k = 0; k < n; k++) for (j = 0; j < count; j++) {
				s = 0;
				for (i = k; i < m; i++) s += qr.get(i, k) * X.get(i, j);
				s = -s / qr.get(k, k);
				for (i = k; i < m; i++) X.set(i, j, X.get(i, j) + s * qr.get(i, k));
			}
			for (k = n - 1; k >= 0; k--) {
				for (j = 0; j < count; j++) X.set(k, j, X.get(k, j) / this.Rdiag[k]);
				for (i = 0; i < k; i++) for (j = 0; j < count; j++) X.set(i, j, X.get(i, j) - X.get(k, j) * qr.get(i, k));
			}
			return X.subMatrix(0, n - 1, 0, count - 1);
		}
		isFullRank() {
			let columns = this.QR.columns;
			for (let i = 0; i < columns; i++) if (this.Rdiag[i] === 0) return false;
			return true;
		}
		get upperTriangularMatrix() {
			let qr = this.QR;
			...
			let i, j;
			for (i = 0; i < n; i++) for (j = 0; j < n; j++) if (i < j) X.set(i, j, qr.get(i, j));
			else if (i === j) X.set(i, j, this.Rdiag[i]);
			else X.set(i, j, 0);
			return X;
		}
		get orthogonalMatrix() {
			...
			let i, j, k, s;
			for (k = columns - 1; k >= 0; k--) {
				for (i = 0; i < rows; i++) X.set(i, k, 0);
				X.set(k, k, 1);
				for (j = k; j < columns; j++) if (qr.get(k, k) !== 0) {
					s = 0;
					for (i = k; i < rows; i++) s += qr.get(i, k) * X.get(i, j);
					s = -s / qr.get(k, k);
					for (i = k; i < rows; i++) X.set(i, j, X.get(i, j) + s * qr.get(i, k));
				}
			}
			return X;
		}
	...
	var SingularValueDecomposition = class {
		constructor(value, options = {}) {
			...
			for (let k = 0; k < mrc; k++) {
				if (k < nct) {
					s[k] = 0;
					for (let i = k; i < m; i++) s[k] = hypotenuse(s[k], a.get(i, k));
					if (s[k] !== 0) {
				...
				if (k < nrt) {
					e[k] = 0;
					for (let i = k + 1; i < n; i++) e[k] = hypotenuse(e[k], e[i]);
					if (e[k] !== 0) {
		...
		inverse() {
			...
			for (let i = 0; i < vrows; i++) for (let j = 0; j < vcols; j++) if (Math.abs(this.s[j]) > e) X.set(i, j, V.get(i, j) / this.s[j]);
			let U = this.U;
			let urows = U.rows;
			let ucols = U.columns;
	...
	function inverse(matrix, useSVD = false) {
		matrix = WrapperMatrix2D.checkMatrix(matrix);
		if (useSVD) return new SingularValueDecomposition(matrix).inverse();
		else return solve(matrix, Matrix.eye(matrix.rows));
	}
	function solve(leftHandSide, rightHandSide, useSVD = false) {
		leftHandSide = WrapperMatrix2D.checkMatrix(leftHandSide);
		rightHandSide = WrapperMatrix2D.checkMatrix(rightHandSide);
		if (useSVD) return new SingularValueDecomposition(leftHandSide).solve(rightHandSide);
		else return leftHandSide.isSquare() ? new LuDecomposition(leftHandSide).solve(rightHandSide) : new QrDecomposition(leftHandSide).solve(rightHandSide);
	}
	function determinant(matrix) {
		matrix = Matrix.checkMatrix(matrix);
		if (matrix.isSquare()) {
			if (matrix.columns === 0) return 1;
			let a, b, c, d;
			if (matrix.columns === 2) {
				a = matrix.get(0, 0);
				b = matrix.get(0, 1);
			...
			} else return new LuDecomposition(matrix).determinant;
		} else throw Error("determinant can only be calculated for a square matrix");
	}
	function xrange(n, exception) {
		let range = [];
		for (let i = 0; i < n; i++) if (i !== exception) range.push(i);
		return range;
	}
	function dependenciesOneRow(error, matrix, index, thresholdValue = 1e-9, thresholdError = 1e-9) {
		if (error > thresholdError) return new Array(matrix.rows + 1).fill(0);
		else {
			let returnArray = matrix.addRow(index, [0]);
			for (let i = 0; i < returnArray.rows; i++) if (Math.abs(returnArray.get(i, 0)) < thresholdValue) returnArray.set(i, 0, 0);
	...
	function linearDependencies(matrix, options = {}) {
		...
		for (let i = 0; i < n; i++) {
			...
			let x = new SingularValueDecomposition(Abis).solve(b);
	...
	function pseudoInverse(matrix, threshold = Number.EPSILON) {
		matrix = Matrix.checkMatrix(matrix);
		if (matrix.isEmpty()) return matrix.transpose();
		let svdSolution = new SingularValueDecomposition(matrix, { autoTranspose: true });
		let U = svdSolution.leftSingularVectors;
		let V = svdSolution.rightSingularVectors;
		let s = svdSolution.diagonal;
		for (let i = 0; i < s.length; i++) if (Math.abs(s[i]) > threshold) s[i] = 1 / s[i];
		else s[i] = 0;
	...
	function covariance(xMatrix, yMatrix = xMatrix, options = {}) {
		...
		if (typeof yMatrix === "object" && !Matrix.isMatrix(yMatrix) && !isAnyArray(yMatrix)) {
			...
			yMatrix = xMatrix;
			yIsSame = true;
		} else yMatrix = new Matrix(yMatrix);
		if (xMatrix.rows !== yMatrix.rows) throw new TypeError("Both matrices must have the same number of rows");
		const { center = true } = options;
		if (center) {
			xMatrix = xMatrix.center("column");
			if (!yIsSame) yMatrix = yMatrix.center("column");
		}
		const cov = xMatrix.transpose().mmul(yMatrix);
		for (let i = 0; i < cov.rows; i++) for (let j = 0; j < cov.columns; j++) cov.set(i, j, cov.get(i, j) * (1 / (xMatrix.rows - 1)));
		return cov;
	}
	function correlation(xMatrix, yMatrix = xMatrix, options = {}) {
		...
		if (center) {
			...
			if (!yIsSame) yMatrix.center("column");
		}
		if (scale) {
		...
		for (let i = 0; i < corr.rows; i++) for (let j = 0; j < corr.columns; j++) corr.set(i, j, corr.get(i, j) * (1 / (sdx[i] * sdy[j])) * (1 / (xMatrix.rows - 1)));
		return corr;
	}
	var EigenvalueDecomposition = class {
		constructor(matrix, options = {}) {
			...
			matrix = WrapperMatrix2D.checkMatrix(matrix);
			if (!matrix.isSquare()) throw new Error("Matrix is not a square matrix");
			if (matrix.isEmpty()) throw new Error("Matrix must be non-empty");
			...
			if (assumeSymmetric) isSymmetric = true;
			else isSymmetric = matrix.isSymmetric();
			if (isSymmetric) {
				for (i = 0; i < n; i++) for (j = 0; j < n; j++) V.set(i, j, value.get(i, j));
				tred2(n, e, d, V);
				tql2(n, e, d, V);
			} else {
				let H = new Matrix(n, n);
				let ort = new Float64Array(n);
				for (j = 0; j < n; j++) for (i = 0; i < n; i++) H.set(i, j, value.get(i, j));
				orthes(n, H, ort, V);
				hqr2(n, e, d, V, H);
			}
			this.n = n;
		...
		get diagonalMatrix() {
			...
			let i, j;
			for (i = 0; i < n; i++) {
				for (j = 0; j < n; j++) X.set(i, j, 0);
				X.set(i, i, d[i]);
				if (e[i] > 0) X.set(i, i + 1, e[i]);
				else if (e[i] < 0) X.set(i, i - 1, e[i]);
			}
			return X;
		}
	};
	function tred2(n, e, d, V) {
		...
		for (j = 0; j < n; j++) d[j] = V.get(n - 1, j);
		for (i = n - 1; i > 0; i--) {
			scale = 0;
			h = 0;
			for (k = 0; k < i; k++) scale = scale + Math.abs(d[k]);
			if (scale === 0) {
				e[i] = d[i - 1];
				for (j = 0; j < i; j++) {
					d[j] = V.get(i - 1, j);
				...
				for (j = 0; j < i; j++) {
					f = d[j];
					g = e[j];
					for (k = j; k <= i - 1; k++) V.set(k, j, V.get(k, j) - (f * e[k] + g * d[k]));
					d[j] = V.get(i - 1, j);
					V.set(i, j, 0);
				}
			}
		...
		for (i = 0; i < n - 1; i++) {
			...
			h = d[i + 1];
			if (h !== 0) {
				for (k = 0; k <= i; k++) d[k] = V.get(k, i + 1) / h;
				for (j = 0; j <= i; j++) {
					g = 0;
					for (k = 0; k <= i; k++) g += V.get(k, i + 1) * V.get(k, j);
					for (k = 0; k <= i; k++) V.set(k, j, V.get(k, j) - g * d[k]);
				}
			}
			for (k = 0; k <= i; k++) V.set(k, i + 1, 0);
		}
		for (j = 0; j < n; j++) {
			d[j] = V.get(n - 1, j);
			V.set(n - 1, j, 0);
	...
	function orthes(n, H, ort, V) {
		...
		for (m = low + 1; m <= high - 1; m++) {
			scale = 0;
			for (i = m; i <= high; i++) scale = scale + Math.abs(H.get(i, m - 1));
			if (scale !== 0) {
				h = 0;
				for (i = high; i >= m; i--) {
				...
				for (j = m; j < n; j++) {
					f = 0;
					for (i = high; i >= m; i--) f += ort[i] * H.get(i, j);
					f = f / h;
					for (i = m; i <= high; i++) H.set(i, j, H.get(i, j) - f * ort[i]);
				}
				for (i = 0; i <= high; i++) {
					f = 0;
					for (j = high; j >= m; j--) f += ort[j] * H.get(i, j);
					f = f / h;
					for (j = m; j <= high; j++) H.set(i, j, H.get(i, j) - f * ort[j]);
				}
				ort[m] = scale * ort[m];
				H.set(m, m - 1, scale * g);
			}
		}
		for (i = 0; i < n; i++) for (j = 0; j < n; j++) V.set(i, j, i === j ? 1 : 0);
		for (m = high - 1; m >= low + 1; m--) if (H.get(m, m - 1) !== 0) {
			for (i = m + 1; i <= high; i++) ort[i] = H.get(i, m - 1);
			for (j = m; j <= high; j++) {
				g = 0;
				for (i = m; i <= high; i++) g += ort[i] * V.get(i, j);
				g = g / ort[m] / H.get(m, m - 1);
				for (i = m; i <= high; i++) V.set(i, j, V.get(i, j) + g * ort[i]);
			}
		}
	}
	function hqr2(nn, e, d, V, H) {
		...
		for (i = 0; i < nn; i++) {
			...
			for (j = Math.max(i - 1, 0); j < nn; j++) norm = norm + Math.abs(H.get(i, j));
		}
		while (n >= low) {
			l = n;
			while (l > low) {
			...
			if (l === n) {
				...
				if (iter === 10) {
					exshift += x;
					for (i = low; i <= n; i++) H.set(i, i, H.get(i, i) - x);
				...
				if (iter === 30) {
					...
					if (s > 0) {
						...
						if (y < x) s = -s;
						s = x - w / ((y - x) / 2 + s);
						for (i = low; i <= n; i++) H.set(i, i, H.get(i, i) - s);
						exshift += s;
						x = y = w = .964;
					}
				}
				...
				while (m >= l) {
					...
					r = r / s;
					if (m === l) break;
					if (Math.abs(H.get(m, m - 1)) * (Math.abs(q) + Math.abs(r)) < eps * (Math.abs(p) * (Math.abs(H.get(m - 1, m - 1)) + Math.abs(z) + Math.abs(H.get(m + 1, m + 1))))) break;
					m--;
				}
				for (i = m + 2; i <= n; i++) {
					H.set(i, i - 2, 0);
					if (i > m + 2) H.set(i, i - 3, 0);
				}
				for (k = m; k <= n - 1; k++) {
					notlast = k !== n - 1;
					if (k !== m) {
					...
					if (p < 0) s = -s;
					if (s !== 0) {
						if (k !== m) H.set(k, k - 1, -s * x);
						else if (l !== m) H.set(k, k - 1, -H.get(k, k - 1));
						p = p + s;
						x = p / s;
						y = q / s;
						z = r / s;
		...
		for (n = nn - 1; n >= 0; n--) {
			...
			if (q === 0) {
				...
				for (i = n - 1; i >= 0; i--) {
					w = H.get(i, i) - p;
					r = 0;
					for (j = l; j <= n; j++) r = r + H.get(i, j) * H.get(j, n);
					if (e[i] < 0) {
						z = w;
						s = r;
					} else {
						l = i;
						if (e[i] === 0) H.set(i, n, w !== 0 ? -r / w : -r / (eps * norm));
						else {
							x = H.get(i, i + 1);
							y = H.get(i + 1, i);
						...
						if (eps * t * t > 1) for (j = i; j <= n; j++) H.set(j, n, H.get(j, n) / t);
					}
				}
			} else if (q < 0) {
				l = n - 1;
		...
		for (i = 0; i < nn; i++) if (i < low || i > high) for (j = i; j < nn; j++) V.set(i, j, H.get(i, j));
		for (j = nn - 1; j >= low; j--) for (i = low; i <= high; i++) {
			z = 0;
			for (k = low; k <= Math.min(j, high); k++) z = z + V.get(i, k) * H.get(k, j);
			V.set(i, j, z);
		}
	}
	...
	var CholeskyDecomposition = class {
		constructor(value) {
			...
			for (j = 0; j < dimension; j++) {
				...
				for (k = 0; k < j; k++) {
					let s = 0;
					for (i = 0; i < k; i++) s += l.get(k, i) * l.get(j, i);
				...
				for (k = j + 1; k < dimension; k++) l.set(j, k, 0);
			}
			this.L = l;
		...
		solve(value) {
			...
			if (value.rows !== dimension) throw new Error("Matrix dimensions do not match");
			if (this.isPositiveDefinite() === false) throw new Error("Matrix is not positive definite");
			let count = value.columns;
			let B = value.clone();
			let i, j, k;
			for (k = 0; k < dimension; k++) for (j = 0; j < count; j++) {
				for (i = 0; i < k; i++) B.set(k, j, B.get(k, j) - B.get(i, j) * l.get(k, i));
				B.set(k, j, B.get(k, j) / l.get(k, k));
			}
			for (k = dimension - 1; k >= 0; k--) for (j = 0; j < count; j++) {
				for (i = k + 1; i < dimension; i++) B.set(k, j, B.get(k, j) - B.get(i, j) * l.get(i, k));
				B.set(k, j, B.get(k, j) / l.get(k, k));
			}
			return B;
		}
	...
	var nipals = class {
		constructor(X, options = {}) {
			...
			let { Y } = options;
			const { scaleScores = false, maxIterations = 1e3, terminationCriteria = 1e-10 } = options;
			let u;
			if (Y) {
				if (isAnyArray(Y) && typeof Y[0] === "number") Y = Matrix.columnVector(Y);
				else Y = WrapperMatrix2D.checkMatrix(Y);
				if (Y.rows !== X.rows) throw new Error("Y should have the same number of rows as X");
				u = Y.getColumnVector(0);
			} else u = X.getColumnVector(0);
			let diff = 1;
			let t, q, w, tOld;
			for (let counter = 0; counter < maxIterations && diff > terminationCriteria; counter++) {
				...
				if (counter > 0) diff = t.clone().sub(tOld).pow(2).sum();
				tOld = t.clone();
				if (Y) {
			...
			if (Y) {
				...
				if (scaleScores) this.t = t.clone().div(this.s.get(0, 0));
				else this.t = t;
...
var AbstractMatrix = import_matrix.AbstractMatrix;
var CHO = import_matrix.CHO;
var CholeskyDecomposition = import_matrix.CholeskyDecomposition;
var DistanceMatrix = import_matrix.DistanceMatrix;
var EVD = import_matrix.EVD;
var EigenvalueDecomposition = import_matrix.EigenvalueDecomposition;
var LU = import_matrix.LU;
var LuDecomposition = import_matrix.LuDecomposition;
var Matrix = import_matrix.Matrix;
var MatrixColumnSelectionView = import_matrix.MatrixColumnSelectionView;
var MatrixColumnView = import_matrix.MatrixColumnView;
var MatrixFlipColumnView = import_matrix.MatrixFlipColumnView;
var MatrixFlipRowView = import_matrix.MatrixFlipRowView;
var MatrixRowSelectionView = import_matrix.MatrixRowSelectionView;
var MatrixRowView = import_matrix.MatrixRowView;
var MatrixSelectionView = import_matrix.MatrixSelectionView;
var MatrixSubView = import_matrix.MatrixSubView;
var MatrixTransposeView = import_matrix.MatrixTransposeView;
var NIPALS = import_matrix.NIPALS;
var Nipals = import_matrix.Nipals;
var QR = import_matrix.QR;
var QrDecomposition = import_matrix.QrDecomposition;
var SVD = import_matrix.SVD;
var SingularValueDecomposition = import_matrix.SingularValueDecomposition;
var SymmetricMatrix = import_matrix.SymmetricMatrix;
var WrapperMatrix1D = import_matrix.WrapperMatrix1D;
var WrapperMatrix2D = import_matrix.WrapperMatrix2D;
var correlation = import_matrix.correlation;
var covariance = import_matrix.covariance;
var matrix_default = import_matrix.Matrix ? import_matrix.Matrix : import_matrix.Matrix;
var determinant = import_matrix.determinant;
var inverse = import_matrix.inverse;
var linearDependencies = import_matrix.linearDependencies;
var pseudoInverse = import_matrix.pseudoInverse;
var solve = import_matrix.solve;
var wrap = import_matrix.wrap;
//#endregion
export { AbstractMatrix, CHO, CholeskyDecomposition, DistanceMatrix, EVD, EigenvalueDecomposition, LU, LuDecomposition, Matrix, MatrixColumnSelectionView, MatrixColumnView, MatrixFlipColumnView, MatrixFlipRowView, MatrixRowSelectionView, MatrixRowView, MatrixSelectionView, MatrixSubView, MatrixTransposeView, NIPALS, Nipals, QR, QrDecomposition, SVD, SingularValueDecomposition, SymmetricMatrix, WrapperMatrix1D, WrapperMatrix2D, correlation, covariance, matrix_default as default, determinant, inverse, linearDependencies, pseudoInverse, solve, wrap };
